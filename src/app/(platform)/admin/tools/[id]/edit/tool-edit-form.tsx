...
export function ToolEditForm({ initial }: { initial: Tool }) {
  ...
  const [installSteps, setInstallSteps] = useState<string[]>(
    initial.installSteps?.length ? initial.installSteps : [""],
  );
        ...
        >
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            ...
            <label className="block text-sm text-neutral-300">
              ...
              <select
                value={contentStatus}
                onChange={(e) => setContentStatus(e.target.value as Tool["contentStatus"])}
                className="mt-1 w-full rounded-lg border border-white/10 bg-black/30 p-2 text-sm outline-none"
              >
